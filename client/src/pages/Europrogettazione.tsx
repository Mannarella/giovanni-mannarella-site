          ...
          <div className="max-w-4xl mx-auto">
                ...
                {programmi.map((programma) => (
                  <Card key={programma.name} className="p-6 hover:shadow-lg transition-shadow">
                    <div className="flex items-center justify-between">
                      ...
                      <a href={programma.url} target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary/80 transition-colors">
            ...
            <section className="bg-card rounded-lg p-8 md:p-12">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-semibold text-foreground mb-2">Programma di Interesse *</label>
                  <select value={formData.opportunita} onChange={(e) => setFormData({ ...formData, opportunita: e.target.value })} className="w-full px-4 py-3 rounded-lg border border-border bg-background text-foreground focus:outline-none focus:ring-2 focus:ring-primary/50" required>
                    ...
                    {programmi.map((programma) => (<option key={programma.name} value={programma.name}>{programma.name}</option>))}
                  </select>
                </div>

                <div className="flex items-start gap-3">
                  <input type="checkbox" id="consenso-europrogettazione" checked={consenso} onChange={(e) => setConsenso(e.target.checked)} className="mt-1 w-4 h-4 accent-primary cursor-pointer shrink-0" required />
                  <label htmlFor="consenso-europrogettazione" className="text-sm text-foreground/60 leading-relaxed cursor-pointer">
